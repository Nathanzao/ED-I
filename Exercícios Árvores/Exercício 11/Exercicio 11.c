...
No* retornaMenorValorArvoreBusca(No *t){ /* Em uma Árvore de Busca, o menor valor está no nó mais a esquerda da árvore. */
    if(t == NULL){
        return NULL;
    }
...
int verificaBusca(No *t, int valorMin, int valorMax){
    if(t == NULL){
        return 1;
    }

    if(t->valor <= valorMin || t->valor >= valorMax){
        return 0;
    }

    int i = verificaBusca(t->esq, valorMin, t->valor);
    if(i != 1){
        return 0;
    }

    int j = verificaBusca(t->dir, t->valor, valorMax);
    if(j != 1){
        return 0;
    }
...
int main() {
    ...
    if (verificaBusca(raiz, -10000, 10000)) {
        printf("A arvore eh uma ABB valida.\n");
    } else {
