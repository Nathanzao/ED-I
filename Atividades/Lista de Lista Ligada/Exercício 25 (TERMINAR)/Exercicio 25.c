...
/*
Crie  uma  rotina  que  permita  inverter  o  sentido  das  referências  de  uma  lista  linear  simplesmente 
encadeada (ou seja, o último nó passa a ser o primeiro da lista).
*/
...
void removeFim(struct Lista *l, struct no *anterior, int *cont){
    anterior->prox = NULL;

    struct no *p = l->ultElem;
...
void inverteOrdem(struct Lista *l){
    struct no *p = l->priElem;
    struct no *aux;
    int lixo;
    ...
    int tamanhos = tamanho(l);

    while(contador != tamanhos){
        struct no *emprestado = p;
        while(emprestado != NULL){
            aux = emprestado;
            emprestado = emprestado->prox;
        }
        int elemInicio = emprestado->elem;
        removeFim(l, aux, &lixo);
        insereNoComeco(l, elemInicio, &lixo);
...
struct Lista *lista2 = (struct Lista*) malloc(sizeof(struct Lista));
    ...
    insereNoComeco(lista, 11, &adicao);
