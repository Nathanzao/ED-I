...
/*
Escreva uma rotina para criar uma cópia de uma lista.
*/
...
void copiaString(struct Lista *l, struct Lista *l2){
    struct no *p = l->priElem;
    ...
    while(p != NULL){
        ...
        if(pp == NULL){ /* Caso a lista 2 acabe, não colocaremos NULL em seus valores. */
            return 0;
        }
...
struct Lista *lista2 = (struct Lista*) malloc(sizeof(struct Lista));
    ...
    copiaString(lista, lista2);

    printf("Lista 1 modificada: \n");
    Imprime(lista);
    printf("\n\n");
